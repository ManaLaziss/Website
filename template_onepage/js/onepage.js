...
var CURRENTLY_SCROLLING = false; //damit nicht zu viele Events überlappen
var ITEMS = null;
...
document.onkeydown = function(event){
	if(event.which == 38) {//up
		...
		event.stopImmediatePropagation(); //stoppt normale Ausführung
...
function softScrollToPage(dir) {//gibt scrollwert der letzten/nächsten elementes mit Klasse "page"
	...
	for (var i=0; i<eles.length; i++) {
		...
		if (dir =="next" && start<pos) {//gibt es ein nächstes Element
			if (tmp.pos==null || pos<tmp.pos) {//neues element ist näher an start
				tmp.pos = pos; //merken
				tmp.i = i;
			}
		}
	...
	if (!CURRENTLY_SCROLLING) softscroll(start, end);
	return end;
}
...
function softscrollTo(anchor) {
	...
	if (end != null && !CURRENTLY_SCROLLING) softscroll(start, end);
}
...
function writeText(){ //stellt auch current item ein
	...
	var no_text = true; //man liegt außerhalb des Textbereich
	for (var i=0; i<eles.length; i++) {
		var pos = 0, par=eles[i];
		while (par != "html" && par != null) {//finde scrollposition
			pos += par.offsetTop;
		...
		if (scroll-pos > 0) {//div-position liegt auf/über scrollposition
			if (tmp.pos==null || scroll-pos<scroll-tmp.pos) {//neues element ist näher an der scrollposition
				tmp.pos = pos; //merken
				tmp.id = eles[i].id; //id übergeben
				no_text = false;
			}
		}
	}
